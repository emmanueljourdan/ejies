/*
 *	ej.lscale by Emmanuel Jourdan, Ircam Ñ 02 2005
 *	scale for lists
 *
 *
 *	$Revision: 1.5 $
...
public class lscale extends ej {
	...
	public void list(float[] args) {
		switch (getInlet()) {
			case 0:                // premire entrŽe... liste ˆ scaler
				a = args;
				calcule();
				break;
			case 1:
				newArgs(args);     // arguments donnŽs sous forme de liste
				break;
			default:               // quelle idŽe d'envoyer une liste ici
				error("ej.lscale: doesn't expect a list here");
				break; // est-ce vraiment nŽcessaire ?
		}
	}
	...
	private void calculeChoice() {
		// choix de la mŽthod pour le scaling
		if (expValue == 1)
